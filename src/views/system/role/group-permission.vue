...
export default {
  ...
  setup(props, ctx) {
    ...
    const moduleCheck = (checked, ids, moduleName) => {
      ...
      if (checked) {
        permissionModuleIds.value = Array.from(
          new Set(permissionModuleIds.value.concat(_ids))
        );
        checkedPermissionNames.value = Array.from(
          new Set(checkedPermissionNames.value.concat(checkedNames))
        );
        if (!permissionModuleNames.value.includes(moduleName)) {
          permissionModuleNames.value.push(moduleName);
        }
      } else {
        permissionModuleIds.value = permissionModuleIds.value.filter(
          v => !_ids.includes(v)
        );
        checkedPermissionNames.value = checkedPermissionNames.value.filter(
          v => !checkedNames.includes(v)
        );
        permissionModuleNames.value = permissionModuleNames.value.filter(
          v => v !== moduleName
        );
      }
      halfPermissions.value = halfPermissions.value.filter(
        v => v !== moduleName
      );
      ctx.emit("updatePermissions", permissionModuleIds.value);
    };
